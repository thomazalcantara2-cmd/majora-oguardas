...
/** Envia um PDF (Buffer) para ser salvo no Drive; devolve a URL do arquivo. */
export async function salvarPdfRecurso(nomeArquivo, bufferPdf) {
  const base64 = bufferPdf.toString('base64');
  const { url } = await chamarAppsScript('salvarPdfRecurso', { nomeArquivo, base64 });
  return url;
}

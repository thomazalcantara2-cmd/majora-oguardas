 ...
 * (ex: { Tentativas_Falhas: 2 }) — não por letra. */
...
/** Procura o PDF com a resposta da SEGEP ao recurso apresentado, na pasta
 * do servidor. Devolve '' se a pasta/arquivo ainda não existir (chamador
 * decide o que mostrar) em vez de lançar erro — essa busca é "best
 * effort". */
export async function obterLinkRespostaRecurso(nome) {
  try {
    const { url } = await chamarAppsScript('obterLinkRespostaRecurso', { nome });
    return url || '';
  } catch (err) {
    return '';
  }
...
/** Procura o PDF do recurso que o próprio servidor apresentou (fase
 * anterior do app), na mesma pasta — se houver mais de um, o Apps Script
 * já devolve o mais recente. Devolve '' se não existir. "Best effort",
 * igual a obterLinkRespostaRecurso. */
export async function obterLinkRecursoApresentado(nome) {
  try {
    const { url } = await chamarAppsScript('obterLinkRecursoApresentado', { nome });
    return url || '';
  } catch (err) {
    return '';
  }

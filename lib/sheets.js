...
/** Envia um PDF (Buffer) para ser salvo como um arquivo novo na pasta do
 * servidor no Drive (nome com carimbo de data/hora — nunca sobrescreve um
 * envio anterior); devolve a URL do arquivo. */
export async function salvarPdfRecurso(nome, bufferPdf, carimbo) {
  const base64 = bufferPdf.toString('base64');
  const { url } = await chamarAppsScript('salvarPdfRecurso', { nome, base64, carimbo });
  return url;
}

/** Envia o anexo do recurso (Buffer) para ser salvo como um arquivo novo,
 * com o mesmo nome-base e carimbo do PDF do recurso (ver
 * salvarAnexoRecurso_ no Apps Script); devolve a URL do arquivo. */
export async function salvarAnexoRecurso(nome, bufferAnexo, nomeArquivoOriginal, tipoMime, carimbo) {

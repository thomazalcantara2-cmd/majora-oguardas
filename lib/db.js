...
export async function registrarTentativaFalha(id, tentativas) {
  if (tentativas >= MAX_TENTATIVAS) {
    ...
    await escreverCelulas(id, { M: tentativas, N: bloqueio });
  } else {
    await escreverCelulas(id, { M: tentativas });
  }
}

export async function zerarTentativas(id) {
  await escreverCelulas(id, { M: 0, N: '' });
}
...
export async function registrarRecurso({ id, matricula, pdfUrl }) {
  await escreverCelulas(id, { K: pdfUrl, L: new Date().toISOString() });

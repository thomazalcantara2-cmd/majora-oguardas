...
export async function obterServidorPorId(id) {
  const linhas = await sql`
    select id, matricula, nome, cpf, ordem, data_requerimento, status,
           data_confirmacao, tentativas_falhas, bloqueado_ate
    from servidores
    where id = ${id}
    limit 1
  `;
...
export async function registrarConfirmacao(id) {
  const linhas = await sql`
    update servidores
  ...
  if (servidor) {
    await sql`
      insert into log_confirmacoes (matricula, tipo)
      values (${servidor.matricula}, 'Confirmação')
    `;
  }
  return servidor || null;
}

export async function registrarRequerimento({ matricula, nome, texto, anexoUrl }) {
  await sql`
    insert into requerimentos (matricula, nome, texto, anexo_url, status_analise)
    values (${matricula}, ${nome}, ${texto || ''}, ${anexoUrl || ''}, 'Pendente')
  `;
}

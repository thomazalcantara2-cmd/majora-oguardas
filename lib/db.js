 ...
 * servidor, nomeados como "RECURSO_<NOME DO SERVIDOR>_<carimbo>" (mesmo
 * nome-base e carimbo de data/hora, cada um com sua extensão). Cada envio
 * gera arquivos novos — nenhum arquivo anterior é apagado ou sobrescrito,
 * só a planilha passa a apontar para os mais recentes. `anexo`, quando
 * enviado, é { buffer, nomeArquivo, tipo }.
 */
export async function registrarRecurso({ id, matricula, nome, bufferPdf, anexo, carimbo }) {
  const pdfUrl = await salvarPdfNoDrive(nome, bufferPdf, carimbo);

  let anexoUrl = '';
  if (anexo && anexo.buffer) {
    anexoUrl = await salvarAnexoNoDrive(nome, anexo.buffer, anexo.nomeArquivo, anexo.tipo, carimbo);
  }
